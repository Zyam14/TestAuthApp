...
## Роли различных файлов:
### users/models.py
Определяет кастомную модель пользователя и связанные сущности (например, UserRole). Содержит поля пользователя и логику вроде is_authenticated

### users/views.py
Реализует API для работы с пользователем: логин (выдача JWT) и soft delete (деактивация пользователя).

### users/urls.py
Описывает маршруты для пользовательских endpoint'ов (например, /login/, /delete/).

### permissions/services.py
Содержит бизнес-логику проверки прав — определяет, может ли пользователь выполнить действие над ресурсом.

### permissions/decorators.py
Декораторы для защиты endpoint’ов — проверяют аутентификацию и права перед выполнением view.

### permissions/urls.py
Маршруты для работы с системой прав (если есть endpoints для управления правилами).

### mock/views.py
Содержит тестовые API endpoints (например, /products/), которые используются для проверки системы прав доступа.

### mock/urls.py
Подключает mock endpoints к маршрутам (/api/mock/...).

### auth_system/settings.py
Главный конфигурационный файл — база данных, middleware, установленные приложения и настройки безопасности.

### auth_system/urls.py
Главный роутер проекта — объединяет все приложения (users, permissions, mock) в единое API.

### middleware
Извлекает пользователя из JWT токена и записывает его в request.user
